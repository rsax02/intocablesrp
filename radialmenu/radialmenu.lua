...
RegisterNetEvent('inventoryOpened')
AddEventHandler('inventoryOpened', function()
inventoryOpened=true
end)

RegisterNetEvent('inventoryClosed')
...
Citizen.CreateThread(function()
    ...
    while true do
        ...
        for _, menuConfig in pairs(menuConfigs) do
            ...
            if menuConfig:enableMenu() and not chatOpened and not inventoryOpened then
                -- When keybind is pressed toggle UI
                local keybindControl = keybindControls[menuConfig.data.keybind]
                if IsControlPressed(0, keybindControl) and not IsControlPressed(0,245) then
                    -- Init UI
                    showMenu = true
                    SendNUIMessage({
                        type = 'init',

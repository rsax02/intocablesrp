...
ESX.GetPlayerPed = function()
	return ped
end

ESX.GetPlayerCoords = function()
	return coords
end

ESX.GetPlayerId = function()
	return id
end
...
ESX.ShowHelpNotification = function(msg, thisFrame, beep, duration)
	AddTextEntry('showHelpNotification', msg .. '~s~.')
	if thisFrame ~= nil and thisFrame then
		DisplayHelpTextThisFrame('showHelpNotification', false)
	else
...
ESX.Game.RequestControlOfEntity = function(entity, cb)
    if not entity then
        return
    end
    local attempt = 0
...
ESX.Game.DeleteEntity = function(entity, request)
    if not entity then
        return
    end
    if request then
...
ESX.Game.Utils.DrawText = function(text, posX, posY, centre)
    SetTextCentre((centre == nil and true or centre))
    SetTextFont(4)
    SetTextScale(0.50, 0.50)
    SetTextColour(255, 255, 255, 255)
    SetTextEntry('STRING')
    AddTextComponentString(text)
    DrawText(posX or 0.5, posY or 0.95)
end
			...
			if data1.current.action == 'give' then
				...
				ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'give_item_to',
				{
					...
					if type == 'item_weapon' then
						...
						TriggerServerEvent('esx:giveInventhecrowsrp2toryItem', GetPlayerServerId(nearbyPlayer), type, item, sourceAmmo)
						menu2.close()
						menu1.close()
						...
						ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'inventory_item_count_give', {
							...
							if quantity ~= nil then
								TriggerServerEvent('esx:giveInventhecrowsrp2toryItem', GetPlayerServerId(nearbyPlayer), type, item, quantity)

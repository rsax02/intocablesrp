...
function takeHostage()
	...
	if not canTakeHostage then 
		drawNativeNotification("Necesitas una pistola con munición para tomar uin rehén!")
	end
...
Citizen.CreateThread(function()
	while true do 
		...
		if holdingHostage then
			...
			DrawText3D(playerCoords.x,playerCoords.y,playerCoords.z,"Presiona [CLICK DER] para soltar, [U] para matar")
			if IsDisabledControlJustPressed(0,25) then --release	
				holdingHostage = false
				holdingHostageInProgress = false 
				local closestPlayer = GetClosestPlayer(2)
				target = GetPlayerServerId(closestPlayer)

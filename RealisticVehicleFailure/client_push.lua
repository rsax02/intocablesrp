    ...
    while true do 
        ...
        if Vehicle.Vehicle ~= nil then
                ...
                if IsVehicleSeatFree(Vehicle.Vehicle, -1) and GetVehicleEngineHealth(Vehicle.Vehicle) <= Config.DamageNeeded then
                    ESX.Game.Utils.DrawText3D({x = Vehicle.Coords.x, y = Vehicle.Coords.y, z = Vehicle.Coords.z}, 'Presiona [~y~SHIFT~w~] y [~y~E~w~] para empujar el vehículo', 0.6)
                end
...
Citizen.CreateThread(function()
    ...
    while true do
        ...
        if (IsControlJustReleased(0, 303) or IsDisabledControlJustReleased(0, 303)) and vehicle ~= nil and vehicle ~= 0 and GetPedInVehicleSeat(vehicle, 0) then
            toggleEngine()
        end
...
function toggleEngine()
    ...
    if vehicle ~= nil and vehicle ~= 0 and GetPedInVehicleSeat(vehicle, 0) then
        SetVehicleEngineOn(vehicle, (not IsVehicleEngineOn(vehicle)), true, true)
    end
end

	...
	while true do
		...
		if showHelp then
			if helpTimer > GetGameTimer() then
				showHelpText("Navega con ~y~W,A,S,D~s~ y confirma con ~y~ESPACIO~s~ para el codigo de la izquierda.")
			elseif helpTimer > GetGameTimer()-helpCycle then
				showHelpText("Usa las ~y~Flechas~s~ y ~y~ENTER~s~ para el codigo de la derecha")
			else

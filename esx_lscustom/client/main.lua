...
function GetAction(data)
	...
	local playerPed = PlayerPedId()
	local vehicle = GetVehiclePedIsIn(playerPed, false)
	local stickers = GetVehicleLiveryCount(vehicle)
		...
		if data.value == k then
			...
			if v.modType then
				...
				elseif v.modType == 48 then
						for j = 1, stickers, 1 do
							if(j==currentMods.modLivery)then
								table.insert(elements, {label = 'Sticker '..j..' - <span style="color:cornflowerblue;">' .. _U('installed') .. '</span>', modType = k, modNum = j, id = j})
							else
								price = math.floor(vehiclePrice*v.price/100)
								table.insert(elements, {label = 'Sticker ' ..j.. ' - <span style="color:green;">$' .. price .. ' </span>', modType = k, modNum = j, id = j})
							end
						end
...
local playerPed = PlayerPedId()
local job, grade, coords, inVehicle = nil, nil, nil, false
Citizen.CreateThread(function()
	while true do
...
Citizen.CreateThread(function()
	while true do
		Citizen.Wait(4)
		if(PlayerData.job and job=="mechanic") and (grade == 'boss' or grade=='experimente' or grade=='chief')then

			if inVehicle then
				local currentZone, zone, lastZone

				if (PlayerData.job and job == 'mechanic') and (grade == 'boss' or grade == 'experimente' or grade == 'chief') or not Config.IsMechanicJobOnly then
					for k,v in pairs(Config.Zones) do

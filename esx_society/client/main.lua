...
function OpenManageEmployeesMenu(society)
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'manage_employees_' .. society, {
		...
		elements = {
			...
			{label = "Empleados en servicio", value='service_list'},
		...
		elseif data.current.value=='service_list'then
			TriggerServerEvent('esx_society:showInService',society)
		end
	...
	ESX.TriggerServerCallback('esx_society:getEmployees', function(employees)
		...
		ESX.UI.Menu.Open('list', GetCurrentResourceName(), 'employee_list_' .. society, elements, function(data, menu)
			...
			elseif data.value == 'fire' then
				...
				ESX.TriggerServerCallback('esx_society:setthecrowsrp2Job', function()
					OpenEmployeeList(society)
...
function OpenRecruitMenu(society)
	ESX.UI.Menu.Open('dialog', GetCurrentResourceName(),'recuit_'..society,{
		title="Introducir Id"
	},function(data,menu)
		local id = data.value
		if id==nil or tonumber(id)==nil then
			ESX.ShowNotification("Id invalida")
			return
		end

		ESX.TriggerServerCallback('esx_society:recruit',function(online,player)
			if online then
				menu.close()
				ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'recruit_confirm_' .. society, {
					title    = _U('do_you_want_to_recruit', player.name),
					align    = 'top-left',
					elements = {
					...
					if data2.current.value == 'yes' then
						ESX.ShowNotification(_U('you_have_hired', player.name))
				...
				ESX.ShowNotification("Ese jugador no esta conectado")
			end 
		end,id)
	end, function(data,menu)
		menu.close()
	...
	--[[ ESX.TriggerServerCallback('esx_society:getOnlithecrowsrp2nePlayers', function(players)
		...
		for i=1, #players, 1 do
			if players[i].job.name ~= society and players[i].job.name~="off"..society then
				table.insert(elements, {
					label = players[i].name.." #"..players[i].id,
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'manage_grades_' .. society, {
			...
			ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'manage_grades_amount_' .. society, {
				...
				elseif amount > Config.MaxSalary then
					...
					ESX.TriggerServerCallback('esx_society:setthecrowsrp2Jobthecrowsrp2Salary', function()
						OpenManageGradesMenu(society)
...
AddEventHandler('esx_society:openBossthecrowsrp2Menu', function(society, close, options)

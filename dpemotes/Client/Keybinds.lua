...
Citizen.CreateThread(function()
  ...
  while true do
    ...
    if NetworkIsPlayerActive(id) and not Initialized then
...
Citizen.CreateThread(function()
 while true do
    ...
    if IsControlJustPressed(2, Config.RagdollKeybind) and Config.RagdollEnabled and IsPedOnFoot(ped) then
        if isInRagdoll then
            isInRagdoll = false
        else
            isInRagdoll = true
    ...
    if #args > 0 then
        ...
        if (Config.KeybindKeys[key]) ~= nil then
        	...
        	elseif DP.PropEmotes[emote] ~= nil then
                ...
                TriggerEvent("chat:addMessage",{args={"^1SYSTEM","^3"..emote.." ^0no es una animacion valido"}})
        	end
        else
            TriggerEvent("chat:addMessage",{args={"^1SYSTEM","^3"..key.." no es un atajo valido"}})

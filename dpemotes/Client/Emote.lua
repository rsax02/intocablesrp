...
RegisterNetEvent("dpemotes:robbering")
AddEventHandler("dpemotes:robbering",function(isRobbering)
  robbering=isRobbering
end)
...
RegisterCommand('e', function(source, args, raw) if not robbering then EmoteCommandStart(source, args, raw) end end)
RegisterCommand('emote', function(source, args, raw) if not robbering then EmoteCommandStart(source, args, raw) end end)
if Config.SqlKeybinding then
...
function HotMenuStart(item)
        local allowed = false
        local ped = PlayerPedId()
        if DP.Hot[item].Car then
        ...
        if 2>1 then
            ...
            for k, v in pairs(GetActivePlayers()) do
                ...
                if src ~= GetPlayerServerId(id) then
                    plr = GetPlayerFromServerId(src)
                    dist = #(GetEntityCoords(ped) - GetEntityCoords(GetPlayerPed(plr)))
                    if not closest then
                        closest = src
                        distance = dist
                    else
            ...
            if distance <= 2.0 then
                TriggerServerEvent('loffe_animations:requestSynced', closest, item)
            else
            ...
            CreateThread(function()
                ...
                if anim['Attach'] then
                    ...
                    AttachEntityToEntity(target, PlayerPedId(), attach['Bone'], attach['xP'], attach['yP'],
                        attach['zP'], attach['xR'], attach['yR'], attach['zR'], 0, 0, 0, 0, 2, 1)
                end
                ...
                if anim['Type'] == 'animation' then
                    LoadDict(anim['Dict'])
                    TaskPlayAnim(target, anim['Dict'], anim['Anim'], 8.0, -8.0, -1, anim['Flags'] or 0, 0, false,
                        false, false)
                end
            ...
            CreateThread(function()
                ...
                if anim['Attach'] then
                    local attach = anim['Attach']
                    AttachEntityToEntity(PlayerPedId(), target, attach['Bone'], attach['xP'], attach['yP'],
                        attach['zP'], attach['xR'], attach['yR'], attach['zR'], 0, 0, 0, 0, 2, 1)
                end

				...
				if hasAccessory then
					TriggerEvent('skinchanger:getSkin', function(skin)
						...
						if skin[_accessory .. '_1'] == mAccessory then
							...
							PlayToggleEmote(Toggle.Emote, function() LastEquipped[which] = false end)
							
						else
							PlayToggleEmote(Toggle.Emote, function() LastEquipped[which] = Cur end)
						end

						local accessorySkin = {}
						accessorySkin[_accessory .. '_1'] = mAccessory
						accessorySkin[_accessory .. '_2'] = mColor
						--PlayToggleEmote(Prop.Emote.On, function() end)
...
function ToggleProps(which)
	...
	if not Prop.Variants then
		...
		if(which=="Glasses" or which=="Hat" or which=="Ear")then
			local accessory = nil
			if(which=="Hat")then
				accessory="Helmet"
				...
				if hasAccessory then
					TriggerEvent('skinchanger:getSkin', function(skin)
						...
						if skin[_accessory .. '_1'] == mAccessory then
							...
							mColor = accessorySkin[_accessory .. '_2']
							--LastEquipped[which] = false
							PlayToggleEmote(Prop.Emote.Off, function() LastEquipped[which] = false end)
							
						else
							PlayToggleEmote(Prop.Emote.On, function() LastEquipped[which] = Cur end)
						end
